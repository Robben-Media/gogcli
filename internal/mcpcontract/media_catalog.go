...
func mediaDefinitions() []Definition {
	...
	return []Definition{
		...
		{Name: "drive_create_file", Description: "Upload bounded caller-supplied bytes as a new Drive file in one multipart request. Requires an explicit write grant; do not replay after an unknown outcome.", Actions: []string{"drive:upload"}, Scopes: []string{drive, driveFile}, AnyScope: true, Retry: NonReplayableWrite},
		{Name: "drive_update_file", Description: "Replace content of the exact Drive file ID with bounded caller-supplied bytes in one multipart request. Requires an explicit write grant; reconcile unknown outcomes before repeating.", Actions: []string{"drive:upload"}, Scopes: []string{drive, driveFile}, AnyScope: true, Retry: NonReplayableWrite},
	}
}
